...
function IngredientOption({ ingrediente, isRemoved, onToggle }: any) {
    if (!ingrediente) return null;
    return (
        <div className="flex items-center gap-2">
            <button
                onClick={() => onToggle(ingrediente.id)}
                className={`flex justify-center items-center w-5 h-5 rounded-sm border ${isRemoved ? 'border-outline-variant text-transparent' : 'bg-primary border-primary text-on-primary'}`}
            >
...
export default function ProductModal({ producto, onClose, onAddToCart }: ProductModalProps) {
    ...
    // Si cerramos y abrimos otro producto, limpiamos el estado
    useEffect(() => {
        setCantidad(1);
            ...
            <div className="flex flex-col md:flex-row h-full max-h-[85vh] overflow-hidden bg-surface">
                ...
                <div className="w-full md:w-1/2 flex flex-col h-full overflow-y-auto filter-scroll relative">
                    ...
                    <div className="p-6 md:p-8 flex-grow flex flex-col gap-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-4">
                                    {ingredientesRemovibles.map(pi => (
                                        <IngredientOption
                                            ...
                                            isRemoved={ingredientesRemovidos.includes(pi.ingrediente_id)}
                                            onToggle={(id: number) => {
                        ...
                        {producto.ingredientes?.some(i => i.ingrediente?.es_alergeno) && (
                    ...
                    <div className="p-6 border-t border-outline-variant bg-surface flex flex-col gap-4 sticky bottom-0 z-10">
                        <div className="flex items-center gap-md">
                            <div className="flex items-center h-[42px] border border-outline-variant rounded bg-white overflow-hidden w-full sm:w-[130px]">
                                <button
                                    ...
                                    className="w-10 h-full flex justify-center items-center text-on-surface hover:bg-surface-container-low transition-colors"
                                >
                        ...
                        <button
                            ...
                            className={`w-full h-[44px] rounded flex justify-center items-center gap-2 text-body-md font-semibold transition-colors duration-200 ${
                                sinStock ? 'bg-surface-variant text-on-surface-variant cursor-not-allowed' : 'bg-primary hover:bg-primary-container text-on-primary'
                            }`}
                        >
